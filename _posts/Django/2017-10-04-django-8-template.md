...
`render` 함수는 두 개의 인자를 필수적으로 받는다. 첫 번째 인자로 `request` 를 받으며, 두 번째 인자로 출력할 템플릿 파일을 받는다.
...
위와 같이 하나의 정적인 템플릿을 만들어 출력하는 방법을 사용해서 데이터를 출력하려면, 글의 수 만큼의 템플릿을 하나 하나 만들어주어야 할 것이다. 다음 포스트에서는 그런 수고를 하지않기 위해 동적으로 필요한 데이터만 바꿔서 여러 템플릿을 생성하는 방법을 알아볼 것이다.
